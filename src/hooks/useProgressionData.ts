    ...
    try {
      ...
      for (const period of periods) {
        ...
        let sessionsQuery = supabase
          .from('quiz_sessions')
          .select('id, total_points, max_points, completed_at, estimated_minutes')
          .eq('team_id', teamId)
          .eq('status', 'completed')
        ...
        const totalTimeSpentMinutes = sessions?.reduce((sum, session) => sum + (session.estimated_minutes || 0), 0) || 0;
