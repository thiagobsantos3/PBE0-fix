...
export function calculateTotalTime(
  ...
  sessions: Array<{ estimated_minutes?: number }> = []
): number {
  let totalTimeMinutes = Math.round(timeSpentSeconds / 60);
  
  // If time_spent is unrealistically low, use estimated_minutes fallback
  if (timeSpentSeconds < 30 && sessions.length > 0) {
    const estimatedMinutesTotal = sessions.reduce((sum, session) => sum + (session.estimated_minutes || 0), 0);
    totalTimeMinutes = estimatedMinutesTotal;
  }
