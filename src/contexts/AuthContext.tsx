...
export function AuthProvider({ children }: { children: ReactNode }) { 
  ...
  const loadUserProfile = async (userId: string, userEmail?: string | null): Promise<User | null> => {
    ...
    try {
      ...
      // Step 5: Fetch team max_members if user has a team
      let maxTeamMembers: number | undefined;
      if (profile.team_id && profile.team_role && ['owner', 'admin'].includes(profile.team_role)) {
        try {
          developerLog('📝 loadUserProfile: Step 5: Fetching team max_members from teams table for team:', profile.team_id);
          ...
          if (!teamError && teamData) {
            maxTeamMembers = teamData.max_members;
            developerLog('✅ loadUserProfile: Step 5: Team max_members fetched:', maxTeamMembers);
          } else {
            developerLog('⚠️ loadUserProfile: Step 5: Could not fetch team max_members:', teamError);
          }
        } catch (error) {
          developerLog('💥 loadUserProfile: Step 5: Error fetching team max_members:', error);
        }
      } else {
        developerLog('ℹ️ loadUserProfile: Step 5: Skipping team max_members fetch - user is not owner/admin or has no team');
      }
